...
async function assertCanViewSubject(user, subjectId, res) {
  ...
  // Student can only view subjects belonging to their class
...
/**
 * Teacher-only:
 * Make sure the teacher owns the lesson.
 */
async function assertOwnsLesson(user, lesson, res) {
  ...
  if (
    ...
    res.status(403);
    throw new Error(
      "You can only modify lessons for your own subjects"
    );
  }
}
...
/**
 * Create a signed Cloudinary delivery URL.
 *
 * IMPORTANT:
 * The backend does NOT download the file.
 * It only creates a secure signed URL.
 *
 * Browser -> Cloudinary
 *
 * instead of:
 *
 * Browser -> Render -> Cloudinary -> Render -> Browser
 */
function createSignedCloudinaryUrl(lesson) {
  if (!lesson.cloudinaryPublicId) {
    return null;
  }

  const resourceType =
    lesson.cloudinaryResourceType || "raw";
...
/**
 * Get a lesson file directly from Cloudinary.
 *
 * This is used for DOWNLOAD fallback / old behavior.
 */
function streamUrlToResponse(
  url,
  res,
  contentType,
  disposition
  ...
  https
    .get(url, (cloudRes) => {
      // Follow redirects from Cloudinary
      if (
        cloudRes.statusCode >= 300 &&
        cloudRes.statusCode < 400 &&
      ...
      // Forward useful headers when available
      ...
      if (cloudRes.headers["accept-ranges"]) {
        res.setHeader(
          "Accept-Ranges",
          cloudRes.headers["accept-ranges"]
        );
      }
...
const createLesson = asyncHandler(async (req, res) => {
  ...
  if (!doc) {
    abort(
      400,
      "A lesson file (PDF, DOCX or PPTX) is required"
    );
  }
  ...
  if (
    ...
    abort(
      403,
      "You can only upload lessons to your own subjects"
    );
  }
  ...
  const lesson = await Lesson.create({
    ...
    cloudinaryResourceType:
      cloudFile.resource_type,

    coverImage: "",
    coverUrl: cloudCover
      ? cloudCover.secure_url
      : "",
    coverPublicId: cloudCover
      ? cloudCover.public_id
      : "",
 ...
 *
 * Download still uses backend streaming.
 * We can optimize download separately if needed.
 */
const downloadLesson = asyncHandler(
  async (req, res) => {
    const lesson = await Lesson.findById(
      req.params.id
    );
    ...
    const safeName = (
      lesson.fileName || "lesson"
    ).replace(/"/g, "");

    const contentType =
      MIME[lesson.fileType] ||
 ...
 *
 * Now:
 * Browser -> Render (permission check)
 * Browser -> Cloudinary (file)
 *
 * Render no longer streams the preview file.
 */
const previewLesson = asyncHandler(
  async (req, res) => {
    const lesson = await Lesson.findById(
      req.params.id
    );
    ...
    if (
      ...
      /**
       * Redirect browser directly to Cloudinary.
       *
       * Render does NOT download the file.
       */
